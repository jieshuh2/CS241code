...
vector *vector_create(copy_constructor_type copy_constructor,
    ...
    vector* created = (vector*)malloc(sizeof(vector));
    created->array = malloc(sizeof(void*) * INITIAL_CAPACITY);
    created->capacity = INITIAL_CAPACITY;
    created->size = 0;
    created->copy_constructor = copy_constructor;
    created->destructor = destructor;
    created->default_constructor = default_constructor;
    return created;
}
...
void vector_resize(vector *this, size_t n) {
    ...
    //     // void** ptr = this->array;
    //     // void** newptr = newarray;
    //     // size_t count = 0;
    //     // while(ptr != NULL) {
    //     //     *newptr = this->copy_constructor(*ptr);
    //     //     newptr++;
    //     //     count++;
    //     //     ptr++;
    //     // }
    //     // while(count < n) {
    //     //     *newptr = this->default_constructor();
    //     //     newptr++;
    //     //     count++;
    //     // }
    //     // vector_clear(this);
    ...
    //     while(this->size < n) {
    //         vector_push_back(this, this->default_constructor());
    //     }
    } else {
        vector_reserve(this, n);
        size_t count = this->size;
        while(count < n) {
            this->array[count] = this->default_constructor();
            count++;
        }
        this->size = n;
    }
...
void vector_reserve(vector *this, size_t n) {
    ...
    if (n <= this->capacity) {
        ...
        size_t newcapacity = get_new_capacity(n);
        this->capacity = newcapacity;
        void** newarray = (void**) malloc(sizeof(void*) * newcapacity);
...
void vector_insert(vector *this, size_t position, void *element) {
    ...
    if (position >= this->size) {
        ...
        size_t newcapacity = get_new_capacity(this->size + 1);
        void** newarray = (void**) malloc(sizeof(void*) * newcapacity);
        this->capacity = newcapacity;
        void** newptr = newarray;
        for (size_t i = 0; i < this->size; i++) {
            if (i == position) {
                *newptr = this->copy_constructor(element);
                newptr ++;
            }
            *newptr = this->array[i];
            newptr ++;
...
void vector_erase(vector *this, size_t position) {
    ...
    assert(position < vector_size(this));
    // your code here
    size_t newcapacity = get_new_capacity(this->size);
    void** newarray = (void**) malloc(sizeof(void*) * newcapacity);
    this->capacity = newcapacity;
    void** newptr = newarray;
    for (size_t i = 0; i < this->size; i++) {
